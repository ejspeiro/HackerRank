...
vector<int> serviceLane(int n, vector<vector<int>> cases, vector<int> widths) {
  vector<int> minWidths{};

  for (auto pairIndices : cases) {
    int minValue{*min_element(widths.begin() + pairIndices.at(0),
                              widths.begin() + pairIndices.at(1) + 1)};
    minWidths.push_back(minValue);
  }
  return minWidths;
}
